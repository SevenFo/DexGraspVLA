...
from dexgraspvla.controller.workspace.base_workspace import BaseWorkspace
from dexgraspvla.controller.policy.dexgraspvla_controller import (
    DexGraspVLAController,
)
from dexgraspvla.controller.dataset.base_dataset import BaseImageDataset
from dexgraspvla.controller.env_runner.base_image_runner import BaseImageRunner
from dexgraspvla.controller.common.checkpoint_util import TopKCheckpointManager
from dexgraspvla.controller.common.json_logger import JsonLogger
from dexgraspvla.controller.model.diffusion.ema_model import EMAModel
from dexgraspvla.controller.model.common.lr_scheduler import get_scheduler
    ...
    def run(self):
        ...
        if accelerator.is_main_process:
            ...
            if torch.cuda.is_available():
                ...
                print(
                    f"CUDA Capability: {torch.cuda.get_device_capability(local_rank)}"
                )
                ...
                if (
                    ...
                    with torch.no_grad():
                        ...
                        if cfg.training.gen_attn_map:
                            train_sample_attn_maps_dir = os.path.join(
                                self.output_dir, "train_sample_attn_maps"
                            )
                            os.makedirs(train_sample_attn_maps_dir, exist_ok=True)
                            output_path = os.path.join(
                                train_sample_attn_maps_dir, f"{self.epoch}.pkl"
                            )
                        else:
                            output_path = None
                        ...
                        if len(val_dataloader) > 0:
                            ...
                            gt_action = batch["action"]
                            if cfg.training.gen_attn_map:
                                val_sample_attn_maps_dir = os.path.join(
                                    self.output_dir, "val_sample_attn_maps"
                                )
                                os.makedirs(val_sample_attn_maps_dir, exist_ok=True)
                                output_path = os.path.join(
                                    val_sample_attn_maps_dir, f"{self.epoch}.pkl"
                                )
                            else:
                                output_path = None
                            pred_action = policy.predict_action(
                                batch["obs"], output_path
                            )
