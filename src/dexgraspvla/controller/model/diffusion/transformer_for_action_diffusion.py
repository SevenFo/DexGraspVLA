...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_attn_mask:
            ...
            masks[
                1, :, obs_part_length[0] : obs_part_length[0] + obs_part_length[1]
            ] = False
    ...
    def forward(
        self, x: torch.Tensor, c: torch.Tensor, training=True, gen_attn_map=False
    ) -> torch.Tensor:
        ...
        if self.fused_attn:
            ...
            if gen_attn_map:
                with torch.no_grad():
                    ...
                    attn_weights = attn_weights[
                        :2, :, 0, :
                    ]  # Only save the first two samples, the first action token
        else:
            q = q * self.scale
            ...
            if gen_attn_map:
                attn_weights = (
                    attn.detach().cpu().to(torch.float16).numpy()
                )  # (B, num_heads, T, L)
                attn_weights = attn_weights[
                    :2, :, 0, :
                ]  # Only save the first two samples, the first action token
...
class RDTBlock(nn.Module):
    ...
    def forward(self, x, c, training=True, gen_attn_map=False):
        ...
        x, attn_weights = self.cross_attn(
            x, c, training=training, gen_attn_map=gen_attn_map
        )
        x = x + origin_x
    ...
    def forward(
        ...
        **kwargs,
        ...
        cond_pos_emb = self.cond_pos_emb[
            :, :tc, :
        ]  # each position maps to a (learnable) vector # (1,N+1,n_emb)
        ...
        for block in self.blocks:
            x, attn_weights = block(
                x, cond_emb, training=training, gen_attn_map=gen_attn_map
            )
            if gen_attn_map:
