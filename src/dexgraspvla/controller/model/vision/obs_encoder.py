...
from dexgraspvla.controller.model.common.module_attr_mixin import ModuleAttrMixin
...
class ObsEncoder(ModuleAttrMixin):
    def __init__(
        ...
        if model_config["head"]["local_weights_path"] is None:
            ...
            self.dino_head.load_state_dict(
                torch.load(model_config["head"]["local_weights_path"])
            )
        ...
        if model_config["wrist"]["local_weights_path"] is None:
            ...
            self.dino_wrist.load_state_dict(
                torch.load(model_config["wrist"]["local_weights_path"])
            )
        ...
        logger.info(
            "Number of parameters in obs encoder: %e",
    ...
    def forward_head(self, rgbm_data, training=True):
        ...
        mask_feature = self.mask_process_net["patch_embed"](
            mask_data
        )  # (B*T, num_patches, head_feature_dim)
        mask_feature = self.mask_process_net["transformer"](mask_feature)

        # Fuse features
        combined_feature = torch.cat([rgb_feature, mask_feature], dim=-1)
        head_feature = self.head_net(
            combined_feature
        )  # (B*T, num_patches, feature_dim)
        head_feature = head_feature.reshape(
            B, T * head_feature.shape[1], head_feature.shape[-1]
        )  # (B, T*num_patches, feature_dim)
    ...
    def forward_wrist(self, wrist_data, training=True):
        ...
        wrist_feature = wrist_feature.reshape(
            B, T * wrist_feature.shape[1], wrist_feature.shape[-1]
        )  # (B, T*num_patches, feature_dim)
    ...
    def forward(self, obs_dict, training=True):
        ...
        embeddings.append(
            self.forward_wrist(obs_dict["right_cam_img"], training)
        )  # (B,T*num_patches,feature_dim)
        embeddings.append(
